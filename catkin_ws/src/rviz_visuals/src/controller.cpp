...
public:
        ...
        : Kp_linear_(kp_linear), Kp_angular_(kp_angular) // Initialize Kp values
    {
        ...
        ackerman_sub_ = nh_.subscribe("/gem/ackermann_cmd", 10, &PathTrackingController::ackermanCallback, this);
...
private:
    ...
    ros::Subscriber ackerman_sub_; // New subscriber for ackermann_cmd
    ...
    {
        steering_angle_ = msg->steering_angle; // Update steering angle
        steering_angle_velocity_ = msg->steering_angle_velocity; // Update steering angle velocity
    }
    ...
    void trackPath(double robot_x, double robot_y, double robot_yaw)
    {
        ...
        if (ros::Time::now() - last_log_time_ >= log_interval_)
        {
            ...
            ROS_INFO("Steering Angle: %.2f, Steering Angle Velocity: %.2f", steering_angle_, steering_angle_velocity_);
            ROS_INFO("-----------------------");
        }
